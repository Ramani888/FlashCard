...
const GlobalLiveFeedScreen = () => {
    ...
    const renderItem = useCallback(
        ({ item }) => (
            <View>
                <View style={[styles.itemContainer, { backgroundColor: colorTheme.listAndBoxColor }]}>
                    <Image
                        source={{ uri: item?.userData?.picture }}
                        style={styles.profileImage}
                    />
                    <Text style={[styles.itemTitle, { color: colorTheme.textColor }]}>Set Name:</Text>
                    <Text style={styles.itemBadge} ellipsizeMode="tail" numberOfLines={1}>
                        {item?.setData?.name}
                    </Text>
                </View>
                <Pressable onLayout={onCardLayout} style={styles.cardContainer}>
                    <View style={[styles.cardHeader, { backgroundColor: colorTheme.cardHeader1 }]}>
                        <Text style={[styles.cardTitle, { color: colorTheme.textColor }]}>{item.top}</Text>
                        <Pressable
                            ref={plusButtonRef}
                            onPress={() => {
                    ...
                    <View style={[styles.cardBody, { backgroundColor: colorTheme.listAndBoxColor }]}>
                        <Text style={[styles.cardDesc, { color: colorTheme.textColor }]}>{item?.bottom}</Text>
                    </View>
                </Pressable>
            </View>
        ),
    ...
    return (
        <View style={[styles.screenContainer, { backgroundColor: colorTheme.background }]}>
